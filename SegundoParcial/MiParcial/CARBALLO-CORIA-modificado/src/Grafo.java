...
public class Grafo {
	...
	public void setArista(int desde, int hasta, int costo, int num_arista){
		grafo.get(desde).add(new Nodo(hasta, costo, num_arista));
	}
			...
			if (sentidoContrarioMin.get(w) != null) {
				for (Nodo aux : sentidoContrarioMin.get(w)) {
					if(predecedentes[aux.getId()] == -1){
						setArista(w, aux.getId(), aux.getCosto(), aux.getNumArista());
					}
				}
			}
